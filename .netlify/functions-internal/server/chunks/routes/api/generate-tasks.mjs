...
const generateTasks = defineEventHandler(async (event) => {
  var _a, _b, _c, _d;
  ...
  function getNextNQuartersFrom(startQuarter, n = 3) {
    ...
    if (!match) return getNextNQuarters(n);
    let year = parseInt(match[1], 10);
    let quarter2 = parseInt(match[2], 10);
    const quarters2 = [];
    for (let i = 0; i < n; i++) {
      let q = quarter2 + i;
      let y = year;
...
User Input: ${customPrompt.trim()}`;
  ...
  for (let i = 0; i < Math.min(3, geminiTasks.length); i++) {
    ...
    const { title, description, due_date, quarter: quarter2 } = t;
    const { data: inserted, error: insertError } = await supabase.from("user_tasks").insert([
      {
        user_id,
        category,
        quarter: quarter2 || quarters[i % quarters.length],
        title: title || "",
  ...
  const { error: creditError } = await supabase.from("user_subscriptions").update({ available_credit: (sub.available_credit || 0) - creditsToDeduct }).eq("user_id", user_id);
  if (creditError) {
...
export { generateTasks as default };
//# sourceMappingURL=generate-tasks.mjs.map
